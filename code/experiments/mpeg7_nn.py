...
result_dir = RESULT_SUBDIRS["mpeg7_nn"]
...
for num_frames in num_frames_div:
    ...
    full_acc_list_nn = {train_mode: [] for train_mode in train_mode_list}
    full_acc_list_nn_running = {train_mode: [] for train_mode in train_mode_list}
    full_acc_list_nn_loss = {train_mode: [] for train_mode in train_mode_list}
    ...
    for run in range(val_fold + 1):
        ...
        for train_mode in train_mode_list:
            ...
            for K in tqdm(K_list):
                ...
                if isCV:
                    cv_acc_list_nn[train_mode][run].append(acc_nn)
                    cv_acc_list_nn_running[train_mode][run].append(acc_nn_running)
                    cv_acc_list_nn_loss[train_mode][run].append(acc_nn_loss)
                    ...
                    full_acc_list_nn[train_mode].append(acc_nn)
                    full_acc_list_nn_running[train_mode].append(acc_nn_running)
                    full_acc_list_nn_loss[train_mode].append(acc_nn_loss)
    ...
    for train_mode in train_mode_list:
        ...
        cv_acc_list_nn[train_mode] = np.array(cv_acc_list_nn[train_mode])
        cv_acc_list_nn_running[train_mode] = np.array(
            cv_acc_list_nn_running[train_mode]
        )
        cv_acc_list_nn_loss[train_mode] = np.array(cv_acc_list_nn_loss[train_mode])
        cv_acc_list_linear[train_mode] = np.array(cv_acc_list_linear[train_mode])
        full_acc_list_nn[train_mode] = np.array(full_acc_list_nn[train_mode])
        full_acc_list_nn_running[train_mode] = np.array(
            full_acc_list_nn_running[train_mode]
        )
        full_acc_list_nn_loss[train_mode] = np.array(full_acc_list_nn_loss[train_mode])
        ...
        np.savez(
            ...
            s_train=nsr,
            cv_acc_nn=cv_acc_list_nn[train_mode],
            cv_acc_nn_running=cv_acc_list_nn_running[train_mode],
            cv_acc_nn_loss=cv_acc_list_nn_loss[train_mode],
            cv_acc_linear=cv_acc_list_linear[train_mode],
            full_acc_nn=full_acc_list_nn[train_mode],
            full_acc_nn_running=full_acc_list_nn_running[train_mode],
            full_acc_nn_loss=full_acc_list_nn_loss[train_mode],
