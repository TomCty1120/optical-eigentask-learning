...
stdz, num_frames, seed = list(product(stdz_list, num_frames_list, list(seed_list)))[
    array_task_id
]

dataset_dir = SPDNN_DIR
result_dir = RESULT_SUBDIRS["spdnn_sim_logistic"]
...
for N in N_list:
    ...
    test_indices = perm[
        np.concatenate(
            [np.where(test_labels[perm] == i)[0][: int(NTest / 10)] for i in range(10)]
        )
    ]
    ...
    print([np.sum(test_labels[test_indices] == i) for i in range(10)])

    data_images = list(np.mean(test_shots[test_indices, :num_frames], axis=1))
    data_train = []
    ...
    for train_mode in train_mode_list:
        ...
        acc_list_logistic = []
        acc_list_logistic_running = []
        acc_list_logistic_loss = []
        acc_list_linear = []
        ...
        for K in tqdm(K_list):
            ...
            acc_list_logistic.append(acc_logistic)
            acc_list_logistic_running.append(acc_logistic_running)
            acc_list_logistic_loss.append(acc_logistic_loss)
        ...
        acc_list_logistic = np.array(acc_list_logistic)
        ...
        np.savez(
            ...
            s_train=nsr,
            acc_logistic=acc_list_logistic,
            acc_logistic_running=acc_list_logistic_running,
            acc_logistic_loss=acc_list_logistic_loss,
